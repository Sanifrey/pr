...
public class Formula9Test {
	...
	@Test
		 /** 
 * Тестируем метод с входными параметрами 120.5, 8.5, 8 и с предполагаемым результатом 52 для проверки работы программы с дробными числами.
 ...
 * Предполагается, что программа выполнит следующие расчёты: 120-8.5*8 и
 * вернёт значение: 52.0.
 */
	public void testFormula1() {
		...
		/**
		 * Выполняем расчёт свободной площади с выше добавленным объектом при значении всей площади "120".
		 * Метод assertEquals сравнивает значения, если значения будут разные, то будет ошибка. Предполагается,что
		 * вернёт значение: 52.0
		 */
		assertEquals("52.0",formula.PFormula("120"));
		/**
