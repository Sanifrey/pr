...
public class Formula10Test {
	...
	@Test
		 /** 
 * Тестируем метод с входными параметрами 121, 8.5, 8 и с предполагаемым результатом 52 для проверки работы программы с дробными числами.
 ...
 * Предполагается, что программа выполнит следующие расчёты: 121-8.5*8 и
 * вернёт значение: 53.0.
 */
	public void testFormula1() {
		...
		/**
		 * Выполняем расчёт свободной площади с выше добавленным объектом при значении всей площади "121".
		 * Метод assertEquals сравнивает значения, если значения будут разные, то будет ошибка. Предполагается,что
		 * вернёт значение: 53.0
		 */
		assertEquals("53.0",formula.PFormula("121"));
		/**
