...
public class Formula11Test {
	...
	@Test
		 /** 
 * Тестируем метод с входными параметрами 111, 8.5, 8 и с предполагаемым результатом 43 для проверки работы программы с дробными числами.
 ...
 * Предполагается, что программа выполнит следующие расчёты: 111-8.5*8 и
 * вернёт значение: 43.0.
 */
	public void testFormula1() {
		...
		/**
		 * Выполняем расчёт свободной площади с выше добавленным объектом при значении всей площади "111".
		 * Метод assertEquals сравнивает значения, если значения будут разные, то будет ошибка. Предполагается,что
		 * вернёт значение: 43.0
		 */
		assertEquals("43.0",formula.PFormula("111"));
		/**
